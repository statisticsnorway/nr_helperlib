...
def rename_multiple_columns(df_in: 'pd.DataFrame', old_substring: str , new_substring: str) -> 'pd.DataFrame':
    """
    Rename multiple columns in a Pandas DataFrame.

    Parameters
    ----------
    df_in : pd.DataFrame
        The input DataFrame to be copied and modified.
    old_substring : str
        The substring to be replaced in the column names.
    new_substring : str
    ...
    -------
    pd.DataFrame
        A new DataFrame with the same data as the input, but with renamed columns.

    Example
    --------
    >>> df = pd.DataFrame({'old_name_a': [1, 2], 'old_name_b': [3, 4], 'old_name_c': [5, 6]})
    >>> df_with_new_names = rename_multiple_columns(df, 'old_name', 'new_name')
    >>> df_with_new_names.columns
    Index(['new_name_a', 'new_name_b', 'new_name_c'], dtype='object')
    """
    
    if not all(isinstance(string, str) for string in [old_substring, new_substring]):
        raise TypeError('old_substring and new_substring must be of type string')
    ...
    # Make series from column headers
    col_names = pd.Series(df.columns)
    
    # Replace old substrings in headers with new ones
    new_names = colnames.str.replace(old_substring, new_substring)
    
    # Construct dictionary with name changes
    rename_dict = dict(zip(col_names, new_names))
